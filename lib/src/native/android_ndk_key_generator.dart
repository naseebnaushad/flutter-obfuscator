...
''';
  ...
  static String _cppSource(
    ...
    buffer.writeln('#include <fstream>');
    buffer.writeln('#include <string>');
...
// ptrace-based tool (gdb, Frida) is attached to this process.
jboolean NativeIsTraced(JNIEnv* /* env */, jobject /* thiz */) {
    ...
    while (std::getline(status, line)) {
        ...
        if (start == std::string::npos) return JNI_FALSE;
        return value[start] != '0' ? JNI_TRUE : JNI_FALSE;
    }
    return JNI_FALSE;
}
...
const JNINativeMethod kMethods[] = {
    ...
    {const_cast<char*>("nativeIsTraced"), const_cast<char*>("()Z"),
     reinterpret_cast<void*>(NativeIsTraced)},
};
...
class ObfuscatorKeyPlugin : FlutterPlugin, MethodChannel.MethodCallHandler {
    ...
    private external fun nativeIsTraced(): Boolean
    ...
    override fun onMethodCall(call: MethodCall, result: MethodChannel.Result) {
        when (call.method) {
            "getKey" -> result.success(nativeMaterializeKey())
            "isTraced" -> result.success(nativeIsTraced())
